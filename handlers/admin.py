...
# handlers/admin.py — ИСПРАВЛЕННАЯ ФУНКЦИЯ ОБНОВЛЕНИЯ ЭКРАНА СЕРВЕРА
...
async def cb_adm_srv_manage(callback: CallbackQuery, db_session: AsyncSession):
    ...
    # ИСПРАВЛЕНО: Добавлен строгий индекс [0] для извлечения ID сервера из массива split
    if callback.data.startswith("adm_toggle_ib_"):
        raw_data = callback.data.replace("adm_toggle_ib_", "")
        server_id = int(raw_data.split("_")[0]) # Четко забираем первый элемент (ID сервера)
    else:
        server_id = int(callback.data.split("_")[-1])
